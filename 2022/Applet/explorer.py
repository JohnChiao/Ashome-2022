...
class Application_UI(object):
	...
	def __init__(self):
		...
		selct_path.add_command(label = "Open...", accelerator="Ctrl + O", command = self.open_dir)
		selct_path.add_command(label = "Save", accelerator="Ctrl + S", command = self.save_file)
		
		menu.add_cascade(label = "File", menu = selct_path)
		
		about = Menu(menu, tearoff = 0)
		about.add_command(label = "Version", accelerator = "V1.1.0")
		about.add_command(label = "Author", accelerator = "John Chiao")
		menu.add_cascade(label = "About", menu = about)
		
		# 顶部frame
		top_frame = Frame(window, bg = "#fff")
		top_frame.pack(side = TOP, fill = X)
		label = Label(top_frame, text = "Current directory: ", bg = "#fff")
		...
		self.folder_img = PhotoImage(file = r"./Resources/fileicon/folder.png")
		self.file_img = PhotoImage(file = r"./Resources/fileicon/text_file.png")
		
		php_img = PhotoImage(file = r"./Resources/fileicon/php.png")
		python_img = PhotoImage(file = r"./Resources/fileicon/python.png")
		image_img = PhotoImage(file = r"./Resources/fileicon/img.png")
		...
		if self.is_file(path) is True:
			...
			if self.is_type_in(path) is False:
				...
				with open(path, "w", encoding = "utf-8") as f:
					f.write(content)
				messagebox.showinfo("Save", "Save success")
			else:
				messagebox.showwarning("Error", "Can't save picture")
		else:
			messagebox.showwarning("Error", "Can't save directory")
			
	''' 设置默认搜索路径'''
	def open_dir(self):
		path = filedialog.askdirectory(title = u"Set directory", initialdir = self.path)
